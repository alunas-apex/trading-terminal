...
export function Orderbook() {
  ...
  const maxQty = Math.max(...asks.map((a) => a.quantity), ...bids.map((b) => b.quantity), 0.001);
    ...
    <div style={{ fontSize: '11px', fontFamily: 'var(--font-mono)', padding: '4px' }}>
      ...
      {asks.map((ask, i) => (
        <div key={`a${i}`} style={{ position: 'relative', display: 'flex', justifyContent: 'space-between', padding: '1px 6px' }}>
          <div style={{ position: 'absolute', right: 0, top: 0, bottom: 0, width: `${(ask.quantity / maxQty) * 100}%`, background: 'var(--red-dim)' }} />
      ...
      <div style={{ padding: '4px 6px', textAlign: 'center', color: 'var(--text-muted)', fontSize: '10px', borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)', margin: '2px 0' }}>
        Spread: {formatPrice(spread)}
      </div>
      {bids.map((bid, i) => (
        <div key={`b${i}`} style={{ position: 'relative', display: 'flex', justifyContent: 'space-between', padding: '1px 6px' }}>
          <div style={{ position: 'absolute', right: 0, top: 0, bottom: 0, width: `${(bid.quantity / maxQty) * 100}%`, background: 'var(--green-dim)' }} />
